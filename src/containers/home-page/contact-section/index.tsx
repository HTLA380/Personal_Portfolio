...
const Contact = () => {
  ...
  const sendEmail = async () => {
    ...
    setLoading(true);
    emailjs
      .sendForm("service_ag3jteg", "template_fqcmgsc", form.current, "htYCUxDW6IaUYA3i2")
      .then(() => reset())
  ...
  return (
    <section id="contact" className="flex flex-col items-center pt-10 pb-20 md:py-48">
      {/* HEADINGS */}
      <AnimatedDiv
        duration={0.5}
        variants={{
        ...
        <h3 className=" font-righteous text-3xl sm:text-5xl text-navy-blue dark:text-light-blue">CONTACT ME</h3>
      </AnimatedDiv>
      ...
      <AnimatedDiv
        ...
        <form onSubmit={handleSubmit(sendEmail)} ref={form} autoComplete="true">
          {errors.name && <p className="text-rose-500 font-dmSans">{errors?.name.message}</p>}
          <input className={inputElClass} type="text" autoComplete="true" placeholder="NAME" {...register("name")} />
          {errors.email && <p className="text-rose-500 font-dmSans">{errors?.email.message}</p>}
          <input className={inputElClass} type="text" placeholder="EMAIL" autoComplete="true" {...register("email")} />

          {errors.message && <p className="text-rose-500 font-dmSans">{errors?.message.message}</p>}
          <textarea
